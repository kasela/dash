(function () {
  ...
  function downloadChartPdf(canvas, title, insight) {
    ...
    var html =
      ...
      '<style>body{font-family:Inter,Arial,sans-serif;margin:28px;color:#0f172a} .head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #e2e8f0;padding-bottom:10px;margin-bottom:18px} .badge{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#6366f1} h1{margin:0;font-size:22px} .grid{display:grid;grid-template-columns:2fr 1fr;gap:16px;align-items:start} .chart{border:1px solid #e2e8f0;border-radius:12px;padding:10px;background:#fff} .insight{border:1px solid #ddd6fe;background:#f5f3ff;border-radius:12px;padding:12px} .insight h2{margin:0 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#6d28d9} .meta{margin-top:18px;color:#64748b;font-size:11px} img{width:100%;height:auto} @media print{body{margin:14px} .meta{position:fixed;bottom:6px;right:8px}}</style></head><body>' +
      '<div class=\"head\"><h1>' + safeTitle + '</h1><div class=\"badge\">Decision Snapshot</div></div>' +
      '<div class=\"grid\"><div class=\"chart\"><img src=\"' + image + '\" alt=\"Chart\"></div>' +
      '<aside class=\"insight\"><h2>AI Insight</h2><p>' + (safeInsight || 'No AI insight available for this chart.') + '</p></aside></div>' +
      '<div class=\"meta\">Generated by DashAI • ' + new Date().toLocaleString() + '</div>' +
      '</body></html>';
  ...
  function exportDashboardPdf() {
    var title = ((document.getElementById('dashboard-title') || {}).textContent || 'Dashboard').trim();
    var cards = document.querySelectorAll('.widget-card');
    var blocks = [];
    cards.forEach(function (card) {
      var clone = card.cloneNode(true);
      clone.querySelectorAll('button, .chart-error, .widget-resize-handle').forEach(function (el) { el.remove(); });
    ...
    var html =
      '<!doctype html><html><head><title>' + safeTitle + '</title>' +
      '<style>@page{size:A4 portrait;margin:14mm} body{font-family:Inter,Arial,sans-serif;color:#0f172a} h1{font-size:24px;margin:0 0 8px} .sub{font-size:12px;color:#64748b;margin-bottom:14px} .grid{display:grid;grid-template-columns:1fr 1fr;gap:12px} .widget-card{break-inside:avoid;border:1px solid #e2e8f0;border-radius:14px;padding:12px;background:#fff} .widget-title{font-size:14px;font-weight:700;margin-bottom:8px;color:#0f172a} .ai-insights-panel{margin-top:8px;border:1px solid #ddd6fe;background:#f5f3ff;border-radius:10px;padding:8px} table{width:100%;border-collapse:collapse} th,td{border:1px solid #e2e8f0;padding:6px;font-size:11px} @media print{.grid{gap:10px}}</style></head><body>' +
      '<h1>' + safeTitle + '</h1>' +
      '<div class=\"sub\">Generated by DashAI • ' + new Date().toLocaleString() + '</div>' +
  ...
  function printHtmlToPdf(html, title) {
    ...
    showToast((title || 'PDF') + ' export ready in print dialog.', 'success');
  }
